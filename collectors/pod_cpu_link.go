...
func (c kubepodCPUCollector) Collect(ch chan<- prometheus.Metric) {
	...
	podCPULinks, err := guaranteedPodCPUs()
...
func getKubernetesCPUList() (string, error) {
	kubeCPUString, err := parseCPUFile("cpuset.cpus")
	if err != nil {
		return "", err
	}
...
func guaranteedPodCPUs() ([]podCPULink, error) {
	...
	if err != nil {
		...
		logFatal("cannot get information on Kubernetes CPU usage, %v", err.Error())
	}
	...
	defaultSet := readDefaultSet(cpuRawBytes)
	links := make([]podCPULink, 0)
	files, err := fs.ReadDir(kubecgroupfs, ".")
	if err != nil {
		return links, fmt.Errorf("could not open path kubePod cgroups: %v", err)
	}

	fileRE := regexp.MustCompile("pod[[:xdigit:]]{8}[_-][[:xdigit:]]{4}[_-][[:xdigit:]]{4}[_-][[:xdigit:]]{4}[_-][[:xdigit:]]{12}")
	cpuSetFileRE := regexp.MustCompile("[[:xdigit:]]{20,}")

	for _, f := range files {
		// Searching for files matching uuid pattern of 8-4-4-4-12
		if match := fileRE.FindString(f.Name()); match != "" {
			podUID := match[3:]
			containerFiles, err := fs.ReadDir(kubecgroupfs, f.Name())
			if err != nil {
				return links, fmt.Errorf("could not read cpu files directory: %v", err)
			}

			for _, cpusetFile := range containerFiles {
				if matches := cpuSetFileRE.MatchString(cpusetFile.Name()); matches {
					cpuSetDesc, err := parseCPUFile(filepath.Join(f.Name(), cpusetFile.Name(), "cpuset.cpus"))
					if err != nil {
						return links, err
					}
...
func parseCPUFile(path string) (string, error) {
	cpuRaw, err := fs.ReadFile(kubecgroupfs, path)
	if err != nil {
